...
async def run_test():
    ...
    try:
        ...
        # -> Open the login page by clicking the 'Client Login' link/button.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/nav/div/div/div[2]/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Open the sign-in form by clicking the 'Sign in' button so the email sign-in mode can be selected.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/div[2]/form/p/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Fill the email field with an invalid email and enter a 6-digit passcode, then submit the form (send Enter) so validation feedback can be observed.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Fill the remaining two passcode inputs with '5' and '6', submit the sign-in form (press Enter), then wait for the UI to update so validation feedback can be observed.
        frame = context.pages[-1]
        # Input text
