...
export default function UserDashboard() {
  ...
  useEffect(() => {
    if (!API_URL) {
      ...
      setFormError('Backend API URL is not configured. Add VITE_API_URL to FRONTEND/.env.local.');
      return;
    }
    ...
    const loadProducts = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setFormError(error?.response?.data?.error || 'Unable to load products right now.');
        }
      } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!API_URL) {
      setFormError('Backend API URL is not configured.');
      return;
    }

    if (!formData.productId) {
      setFormError('Select a product before submitting.');
      return;
    }
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <h1 className="text-3xl font-semibold sm:text-4xl" style={{ color: 'var(--color-text)' }}>
                {user ? `Welcome back, ${user.name.split(' ')[0]}!` : 'My Dashboard'}
              </h1>
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                ...
                { icon: Phone, label: 'Phone', value: user.phone || 'Not set yet' },
                ...
                >
                  ...
                  <div>
                    <div className="mb-1 text-xs uppercase tracking-[0.2em]" style={{ color: 'var(--color-muted)' }}>
                      {label}
                    </div>
        ...
        <section className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-medium uppercase tracking-[0.2em]" style={{ color: 'var(--color-muted)' }}>
                  Product
                </label>
                <select
                  required
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder="Any delivery or pickup instructions..."
          ...
          >
            <div className="mb-6">
              ...
              <p className="text-sm" style={{ color: 'var(--color-muted)' }}>
                Orders linked to your account load automatically.
              </p>
            </div>
                ...
                requests.map((request) => (
                  ...
                  >
                    ...
                    {request.notes ? (
                      ...
                      >
                        <strong style={{ color: 'var(--color-text)' }}>Notes:</strong> {request.notes}
                      </div>
                    ) : null}
                  </article>
                ))
